...
public partial class MainWindow
{
    ...
    private Control Dashboard()
    {
        ...
        var values = new[] { ("Customers", Model.Customers.Count.ToString(), "people", "#1565C0"), ("Products", Model.Products.Count.ToString(), "inventory_2", "#2E7D32"), ("Invoices", Model.Invoices.Count.ToString(), "receipt_long", "#E65100"), ("Revenue Collected", "Rs. " + Model.Invoices.Sum(i => decimal.TryParse(i["Paid"], out var n) ? n : 0).ToString("0.00"), "account_balance_wallet", "#6A1B9A"), ("Outstanding", "Rs. " + Model.Invoices.Sum(i => decimal.TryParse(i["Outstanding"], out var n) ? n : decimal.TryParse(i["Total"], out var total) ? total : 0).ToString("0.00"), "hourglass_top", "#C62828") };
