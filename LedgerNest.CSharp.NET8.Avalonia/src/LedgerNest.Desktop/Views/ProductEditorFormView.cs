...
internal sealed class ProductEditorFormView : UserControl
{
    ...
    public ProductEditorFormView(FormField[] fields, Func<string, bool> visible)
    {
        ...
        void Section(string title, params (string Label, Control Control)[] rows)
        {
            ...
            if (shown.Length > 0) body.Children.Add(Ui.Stack(12, Ui.Text(title, 11, true, Ui.Muted), Ui.Stack(16, shown)));
        }
        ...
        Section("GENERAL", ("Name", Input("Name", "inventory_2")),
            ("Alias Name", Ui.Stack(6, Input("Alias Name (for invoice PDF)", "translate"), aliasHelp)),
        ...
        Section("PRICING", ("Price", Pair("Sale Price", Input("Sale Price"), "Purchase Price", Input("Purchase Price"))),
            ...
            ("Tax Rate", Ui.Stack(2, Pair("Tax (%)", Input("Tax (%)", "percent"), "Price includes tax", Check(F("Price includes tax"))),
                Ui.Text("Per-item tax mode only", 12, color: Ui.Muted))));
        ...
        {
            var header = Ui.Columns("24,12,*", Ui.Icon("more_horiz", 22, Purple), new Border(), Ui.Text("Advanced Information", 15));
            ...
            expander.Template = new Avalonia.Controls.Templates.FuncControlTemplate<Expander>((owner, _) =>
            {
                ...
                var toggle = new ToggleButton { Content = Ui.Columns("*,Auto", header, arrow),
                    ...
                    Background = Brushes.Transparent, BorderThickness = new Thickness(0), Padding = new Thickness(0, 8), MinHeight = 44 };
        ...
        body.Children.Add(new Border { Background = Ui.Palette("#FFF5ED", "#3B3026"), BorderBrush = Brush.Parse("#FFD18B"), BorderThickness = new Thickness(1),
            CornerRadius = new CornerRadius(10), Padding = new Thickness(12), Child = Ui.Columns("16,10,*", Ui.Icon("lightbulb", 16, Brush.Parse("#FFB300")), new Border(),
                Ui.Text("Tip: Choose optional product fields in Settings → Product Details.", 12, color: Ui.Muted)) });
        Content = body;
    }

    private static Control Check(FormField field, string help = "")
    {
        var check = new CheckBox { Content = help.Length == 0 ? Ui.Text(field.Label, 13) : Ui.Stack(3, Ui.Text(field.Label, 13), Ui.Text(help, 12, color: Ui.Muted)) };
    ...
    internal static Control ProductInput(FormField field, string icon)
    {
        ...
        {
            var combo = new ComboBox { ItemsSource = field.Options, HorizontalAlignment = HorizontalAlignment.Stretch, MinHeight = 48,
                Padding = new Thickness(icon.Length == 0 ? 12 : 42, 8, 12, 8), Background = Brushes.Transparent, CornerRadius = new CornerRadius(10) };
            combo.Bind(SelectingItemsControl.SelectedItemProperty, binding); input = combo;
        }
        else
        {
            var text = new TextBox { PlaceholderText = field.Label, MinHeight = field.Kind == "multiline" ? 96 : 48,
                AcceptsReturn = field.Kind == "multiline", TextWrapping = field.Kind == "multiline" ? TextWrapping.Wrap : TextWrapping.NoWrap,
                Padding = new Thickness(icon.Length == 0 ? 12 : 42, field.Kind == "multiline" ? 26 : 12, 12, 12), FontSize = 15,
            ...
            if (field.Kind == "date")
            {
                ...
                flyout.Content = Ui.Stack(8, calendar, Ui.Button("Clear date", () => { field.Value = ""; calendar.SelectedDate = null; flyout.Hide(); }));
        ...
        var label = new Border { Background = Surface, Padding = new Thickness(4, 0), Margin = new Thickness(9, -7, 0, 0),
    ...
    internal static Control TypeSelector(FormField type)
    {
        ...
        foreach (var value in type.Options)
        {
            ...
            button.Tag = value; button.Width = 104; button.Height = 32; button.MinHeight = 32;
            button.Padding = new Thickness(8, 4); button.CornerRadius = new CornerRadius(0);
