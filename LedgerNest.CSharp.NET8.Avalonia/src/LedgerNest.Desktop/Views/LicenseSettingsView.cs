...
public partial class MainWindow
{
    private Control LicenseWorkspaceContent(Control body)
    {
        ...
        var caption = Ui.Text("", 13);
        var banner = new Border { Padding = new Thickness(16, 8), Background = Ui.Palette("#FFF4E3", "#3F321E"), Child = Ui.Columns("*,12,Auto", caption, new Border(), manage) };
    ...
    private Control LicenseSettingsView()
    {
        ...
        void RenderStatus()
        {
            ...
            var fields = Ui.Stack(12, Ui.Text(title, 22, true), Ui.Text(license.Message, 14));
            if (license.Claims is { } claims)
            {
                fields.Children.Add(Ui.Text("Licensed to: " + claims.Customer, 15, true));
                fields.Children.Add(Ui.Text("License ID: " + claims.LicenseId, 12, color: Ui.Muted));
                fields.Children.Add(Ui.Text("Expires: " + (claims.ExpiresAtUtc?.ToLocalTime().ToString("dd MMM yyyy HH:mm zzz") ?? "Perpetual"), 14));
            }
        ...
        var device = new TextBox { Text = model.LicenseDeviceId, IsReadOnly = true, MinHeight = 46, TextWrapping = TextWrapping.Wrap };
        ...
        var paste = new TextBox { PlaceholderText = "Paste a license document", AcceptsReturn = true, TextWrapping = TextWrapping.Wrap, MinHeight = 120, MaxLength = LicenseFeatures.MaximumFileBytes };
        ...
        var content = Ui.Stack(20, summary,
            Ui.Card(Ui.Stack(12, Ui.Text("Activate this device", 18, true),
                Ui.Text("Send this Device ID to your software provider to receive a trial, paid or renewal license.", 14, color: Ui.Muted),
                device, copy, import), 22),
            Ui.Card(Ui.Stack(12, Ui.Text("Or paste your license", 18, true), paste, activate), 22),
