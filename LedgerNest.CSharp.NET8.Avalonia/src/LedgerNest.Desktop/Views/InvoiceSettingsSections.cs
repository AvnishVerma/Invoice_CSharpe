...
{
    private Control InvoiceItemSettings() => Ui.Stack(16,
    ...
    private Control InvoiceColumnSettings()
    {
        ...
        var description = InvoiceSettingsCard(Ui.Stack(6.4,
    ...
    private Control CustomFieldsPreview(bool full = false)
    {
        ...
        for (var i = 0; i < fields.Length; i++)
        {
            ...
            var sample = Ui.LocalText("Sample value", full ? 14 : 11, color: Ui.Muted); sample.FontStyle = FontStyle.Italic;
            var cell = new Border { Padding = new Thickness(4.8), BorderBrush = Ui.Palette("#EAE4EE", "#526171"), BorderThickness = new Thickness(1), Child = Ui.Stack(2.4, label, sample) };
    ...
    private Control InvoiceCustomSettings()
    {
        ...
        void Render()
        {
            ...
            for (var i = 0; i < Model.InvoiceCustomFieldDefinitions.Count; i++)
            {
                ...
                Button Action(string title, string icon, Action action)
                {
                    ...
                    button.Classes.Add("text"); button.Padding = new Thickness(3.2); button.MinWidth = 30;
        ...
        var newLabel = new TextBox { PlaceholderText = "New field label", MinHeight = 38.4, MaxLength = 100 };
        AutomationProperties.SetName(newLabel, "New field label");
        void Add() { if (Model.AddInvoiceCustomField(newLabel.Text ?? "")) newLabel.Text = ""; }
        var add = Ui.Button("Add custom field", Add, true); add.Content = Ui.LocalText("＋ Add", 14, true, Brushes.White); add.Background = Brush.Parse("#6750A4"); add.MinHeight = 25.6; add.CornerRadius = new CornerRadius(18);
        newLabel.KeyDown += (_, e) => { if (e.Key == Avalonia.Input.Key.Enter) { Add(); e.Handled = true; } };
        var previewCard = InvoiceSettingsCard(Ui.Stack(12.8, Ui.Columns("16,8,*", Ui.Icon("visibility", 16), new Border(), Ui.LocalText("Preview", 13)), livePreview,
            Ui.LocalText("ⓘ  Preview may slightly differ in the final PDF.", 12, color: Ui.Muted)), 16);
        previewCard.Background = Ui.Palette("#FDF7FF", "#25212B");
        var definitionEditor = Ui.Stack(12.8, rows, Ui.Columns("*,8,Auto", newLabel, new Border(), add), previewCard);
