...
public static class FormCatalog
{
    ...
    public static Dictionary<string, FormSection[]> Settings() => new()
    {
        ["Company Info"] = [
            ...
            new("BUSINESS TYPE", [new("Business Type", "Both", "choice", ["Product", "Service", "Both"])]),
            new("PAYMENT SETTINGS", [Toggle("Show QR code on invoice", false, "Adds scannable UPI payment QR codes to generated PDFs"), Toggle("Show bank details on invoice", false, "Prints bank account details on generated PDFs")]),
        ...
        ["Invoice Settings"] = [
            ...
            new("Branding", [Choice("Logo Position", "Left", "Right"), Number("Logo Size", "90"), new("Signature Image", kind: "file"), Number("Signature Size", "50"), Choice("Signature Position", "Right", "Left"), new("Watermark Image", kind: "file"), new("Watermark Opacity", "15", "slider")]),
            new("Columns", [Toggle("Show Sl. No.", true), Toggle("Item Name", true), Toggle("HSN/SAC", true), Toggle("Show Quantity", true), Toggle("Price", true), Toggle("Tax", true), Toggle("Show Discount", true), Toggle("Total", true), .. MainWindowViewModel.InvoiceMetadataLabels.Select(label => Toggle("Metadata: " + label))]),
