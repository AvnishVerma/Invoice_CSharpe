...
Workspace-access batch: database-backed startup, logout, route rendering and keyboard shortcuts now respect session state and mandatory password changes. The no-database preview model remains usable for layout checks. Build passed with zero warnings/errors; 502 checks passed using local runtime major roll-forward. Service-level authorization, first-run owner setup, credential hash migration and recovery remain incomplete. Hosted CI for prior commit cd0ff9b passed: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34316511091 .

Credential-storage batch: new credentials use versioned PBKDF2-HMAC-SHA256 with random salts and fixed-time verification; successful verification upgrades existing C# SHA256 credentials. All 516 local checks passed with runtime major roll-forward; build had zero warnings/errors. See PASSWORD_STORAGE.md for format, compatibility, performance evidence and outstanding review. Prior workspace-lock commit 2965531 passed hosted CI: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34316871484 . This batch awaits its hosted run.

Session-revalidation batch: verified account snapshots are checked on activation, page rendering, keyboard commands and current-password changes. External identity, role, credential, required-change and deletion changes invalidate access. Login now establishes identity from the verified record rather than a second lookup. Build passed with zero warnings/errors and 531 local checks passed using runtime major roll-forward. Service authorization, continuous/inactivity locking and concurrent business-operation guards remain open. Prior credential commit 3dfbb43 passed hosted CI: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34317138062 . This batch awaits its hosted run.

Safe database-restore batch: removed live-file deletion before validation. Candidates are checked in isolation and copied using the SQLite backup API. Regression coverage found missing customer FK enforcement; explicit relationship checks now reject that case. Build passed with zero warnings/errors; 553 local checks passed using runtime major roll-forward. See DATABASE_RESTORE.md for limits and pending failure drills. Prior session commit d70b81f passed hosted CI: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34317394098 . This batch awaits its hosted run.

JSON restore validation batch: empty objects can no longer erase business data. Full table arrays, valid IDs/settings keys and consistent references are checked before replacement. Build passed with zero warnings/errors; 591 local checks passed using runtime major roll-forward. Financial consistency and full recovery/authorization gates remain open. Prior database-restore commit 4a6e9ba passed hosted CI: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34317728917 . This batch awaits its hosted run.

JSON recovery-drill batch: database setup/transaction creation moved inside restore error handling. An injected SQLite insert failure verifies rollback after deletion and successful retry; an injected context-creation failure verifies graceful failure and retry. Build passed with zero warnings/errors; 600 local checks passed using runtime major roll-forward. Power-loss, disk-full and concurrent-write drills remain open. Hosted CI for prior commit 171822f was still running when checked: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34347168506 .

Destination-lock restore batch: restore connections use a two-second SQLite lock timeout. A competing write transaction test verifies preservation of newer live data/session, staging cleanup and retry after lock release. The locked attempt returned in 10 ms locally. Build passed with zero warnings/errors; 609 local checks passed using runtime major roll-forward. Disk-full, power-loss and broader concurrency drills remain pending. Prior commit a6e8536 hosted CI was still running when checked: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34347347653 .

Restore-outcome batch: successful copy remains successful when later workspace reload fails, with restart guidance and cleared session. Cleanup IO/access failures no longer replace the copy outcome. Build passed with zero warnings/errors; 615 local checks passed using runtime major roll-forward. Platform cleanup-failure injection remains unverified. Prior commit 1ba766a passed hosted CI: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34347538359 . This batch awaits its hosted run.

JSON restore-outcome batch: reload failures after a committed JSON transaction now preserve the successful restore result and provide restart guidance while clearing access. A post-commit context failure confirms committed data and reopening. Build passed with zero warnings/errors; 621 local checks passed using runtime major roll-forward. Prior bd6df28 hosted CI passed: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34347835255 . This batch awaits its hosted run; full authorization and recovery gates remain open.

Pending backup-operation batch: UI callbacks retain their starting model/session version and revalidate before continuing after asynchronous picker/IO steps. Tests cover session validity and invalidation; native picker automation and service authorization remain open. Build passed with zero warnings/errors; 629 local checks passed using runtime major roll-forward. Prior ef44c4a hosted CI passed: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34348446896 . This batch awaits hosted CI.

Backup export-stream batch: seekable streams are reset/truncated and flushed/disposed before success reporting. UI backup file actions handle IO/access failures. Build passed with zero warnings/errors; 633 local checks passed using runtime major roll-forward. Atomic output replacement and actual disk-full/native-provider drills remain open. Prior d2ebf8a hosted CI passed: https://github.com/AvnishVerma/Invoice_CSharpe/actions/runs/34348710604 . This batch awaits hosted CI.

User-reported UI issues: implemented all 11 rows pasted on 2026-09-10, covering login feedback/case/Enter, administrator setup, dashboard layouts, default customer, selection re-entry, embedded logo persistence, toggle appearance, action dropdowns and payment completion. Zero-warning build and 670 headless/database checks passed. Five issue captures visually reviewed. Spreadsheet statuses are supplied as REPORTED_ISSUES_STATUS.csv because direct Google Sheets access is unavailable. Native picker checks and overall production-readiness gates remain incomplete.
