...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        Click("Cancel");
        Check(model.Lines.Count == 0, "Cancelling an item dialog must leave the invoice unchanged");
        searchBox.Text = "SCAN-001";
        searchBox.RaiseEvent(new KeyEventArgs { RoutedEvent = InputElement.KeyDownEvent, Key = Key.Enter }); Settle();
        TextBox ItemField(string label) => window.GetVisualDescendants().OfType<TextBox>().Single(t => t.PlaceholderText == label);
        ItemField("Quantity").Text = "0"; Click("Add");
        Check(model.Lines.Count == 0, "Item dialog must reject zero quantity");
        ItemField("Quantity").Text = "2";
        ItemField("Unit Price (override)").Text = "3500";
        ItemField("Discount").Text = "10";
        ItemField("Extra Cost (optional)").Text = "25";
        Click("Add");
        Check(model.Lines.Count == 1, "Confirming the scanned item must add exactly one line");
        Check(model.Lines.Single() is { Quantity: 2, Price: 3500, Discount: 10, DiscountPerUnit: true, ExtraCost: 25, Unit: "pcs" }, "Item dialog overrides must reach the invoice");
        Check(model.Lines.Single().Total == 7005m, "Item dialog must apply per-unit discount and flat extra cost");
        model.Lines.Clear();
        ...
        Click("Cancel");
        Check(window.GetVisualDescendants().OfType<TextBox>().Any(t => t.PlaceholderText == "Current Password"), "Mandatory password change cannot be dismissed");
        foreach (var field in window.GetVisualDescendants().OfType<TextBox>())
        {
            if (field.PlaceholderText == "Current Password") field.Text = "admin";
            if (field.PlaceholderText is "New Password (min 8 characters)" or "Confirm New Password") field.Text = "updated-admin-password";
        }
        ...
        window.GetVisualDescendants().OfType<TextBox>().First(t => t.PlaceholderText == "Company Name").Text = "Issue review company";
