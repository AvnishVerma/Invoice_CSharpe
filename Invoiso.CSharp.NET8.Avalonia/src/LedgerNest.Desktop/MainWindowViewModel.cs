...
public partial class MainWindowViewModel : ObservableObject
{
    ...
    public bool SaveRecord(string kind, FormField[] fields, UiRecord? original = null)
    {
        ...
        var record = new UiRecord { Values = fields.Where(f => f.Kind != "password").ToDictionary(f => f.Label, f => f.Kind == "toggle" ? f.IsChecked.ToString() : f.Value.Trim()) };
        if (original != null) records[records.IndexOf(original)] = record; else records.Add(record);
        Status = $"{kind} saved for this session.";
        return true;
    }
    public bool SaveInvoice()
    {
        ...
        Invoices.Add(new UiRecord { Values = new() {
        ...
        Status = $"{InvoiceDetails[0].Value} saved for this session.";
        return true;
    }
}
